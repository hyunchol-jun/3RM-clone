...
import { sendMessageToChat, downloadPhoto } from "../../utils/apiCalls";
...
export default function ChatDetailBody({
  ...
  const [userPhotos, setUserPhotos] = useState<{ [key: string]: any }>({});

  useEffect(() => {
    if (users) {
      users.forEach((user) => {
        downloadPhoto(user.id, user.photo.photo_id).then((res: any) => {
          setUserPhotos((prev) => {
            const copiedPhotos = { ...prev };
            copiedPhotos[user.id] = res.bytes;
            return copiedPhotos;
          });
        });
      });
      ...
      <div className="w-full">
        <div className="shadow-lg rounded-xl">
          ...
          <div className="flex flex-col gap-4 bg-white px-6 py-6 rounded-b-xl">
            {isEditorShown && (
              <Note name="You" time="Just now" photo={userPhoto}>
                ...
                return (
                  <Note
                    ...
                    photo={
                      ...
                      userPhotos[message.from_id.user_id]
                    }
                  >
